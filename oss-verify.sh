...
#
# Options:
#   --repo        GitHub repo in owner/repo format, e.g. aquasecurity/trivy (required)
#   --version     Exact version to install, e.g. 0.70.0 (required, no auto-fetch)
#   --binary      Binary name to extract (default: repo name, then auto-detected)
...
#   --no-install  Verify only, do not install
...
#   ./oss-verify.sh --repo anchore/grype --version 0.88.0 --cutoff 2026-03-01
#   ./oss-verify.sh --repo cli/cli --binary gh --version 2.49.0 --dry-run
#
# Signing patterns supported (auto-detected):
#   A  direct_bundle    <binary>.sigstore.json present
#   B  checksum_certsig checksums file + .pem + .sig present
#   C  checksum_bundle  checksums file + .sigstore.json present
#   D  checksum_only    checksums file only (no cosign — integrity only, not provenance)
#
# Limitations:
#   Cannot protect against an attacker with live CI credentials publishing a fresh
#   malicious release — cosign will pass because the signature is legitimate.
#   The lockfile protects re-installs of previously verified versions only.
#   Human process (monitoring advisories, not auto-upgrading) is the last line
#   of defence for that scenario.
#
# Security properties:
#   - cosign verification always pins both identity AND OIDC issuer — no issuer-free fallback
#   - Identity extraction failure aborts — no broad fallback regexp
#   - grep in verify_checksums anchored to prevent short-name substring matches
#   - Binary presence in checksums confirmed before sha256sum --ignore-missing is trusted
#   - LOCK_DIR and INSTALL_DIR validated as absolute paths, no .. , no system dir prefixes
#   - OSS_VERIFY_LOCK_DIR env var validated by validate_path before use
#   - Temp dir locked to 700 immediately after creation
#   - Archive extraction sandboxed — path traversal entries (including bare ..) rejected
#   - find after extraction rejects symlinks explicitly and path-escape checked
#   - Both api_curl and binary_curl use --proto "=https" — no protocol downgrade possible
#   - Asset name pattern detection uses literal/suffix/endswith — no regexp on API data
#   - Signing epoch 0 or empty aborts rather than writing a meaningless lockfile entry
#   - validate_path checks full prefix, not just exact match, against forbidden dirs
#   - Lockfile ownership verified before reading; stat failure distinguished from mismatch
#   - Argument parsing checks $# before shift 2 — no unbound variable on missing values
#   - base64_decode captures output explicitly — empty decode result aborts extraction
#   - TRUST_CUTOFF_DATE validated as strict YYYY-MM-DD before passing to date -d
#   - BINARY_FILENAME validated for safe characters before raw binary install
#   - id -u captured once at start, not in subshell inside comparison
#   - RELEASE_JSON piped via printf not echo — no shell interpretation of content
#   - --wildcards (GNU-only) removed from tar — portable extraction on Linux and macOS
#   - find_asset_url_endswith uses clean glob suffix match — dead inner check removed
